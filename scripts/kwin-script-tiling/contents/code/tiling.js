...
function Tiling(layoutType, desktop, screen) {
    try {
        ...
        var gapSize = KWin.readConfig("gapSize", 8);  /* stick to old gaps config by default */
        ...
        this.screenGapSizeLeft = KWin.readConfig("screenGapSizeLeft", 8);
        this.screenGapSizeRight = KWin.readConfig("screenGapSizeRight", 8);
        this.screenGapSizeTop = KWin.readConfig("screenGapSizeTop", 8);
        this.screenGapSizeBottom = KWin.readConfig("screenGapSizeBottom", 8);
    } catch(err) {
        print(err, "in Tiling");
    }
};
...
Tiling.prototype.applyGaps = function(rect) {
    if (KWin.readConfig("fullscreenGaps", true) == false && util.compareRect(rect, this.screenRectangle) == true) return;
